...
def enviar_pontuacao(score):
    try:
        response = requests.post('http://127.0.0.1:5003/update-score', json={'score': score})
...
def atualizar_pergunta_atual():
    ...
    try:
        response = requests.get('http://127.0.0.1:5003/get-current-question')
...
def proxima_pergunta():
    ...
    current_question += 1
    try:
        requests.post('http://127.0.0.1:5003/next-question', json={'current_question': current_question})
    except requests.exceptions.RequestException as e:
