...
def enviar_pontuacao(score):
    try:
        response = requests.post('http://127.0.0.1:5002/update-score', json={'score': score})
...
def atualizar_pergunta_atual():
    ...
    try:
        response = requests.get('http://127.0.0.1:5002/get-current-question')
...
def proxima_pergunta():
    ...
    current_question += 1
    try:
        requests.post('http://127.0.0.1:5002/next-question', json={'current_question': current_question})
    except requests.exceptions.RequestException as e:
        print(f"Erro ao avançar para a próxima pergunta: {e}")

questions = [
    {"question": "Qual é a letra E?", "answer": "E"},
    {"question": "Qual é a letra F?", "answer": "F"},
    {"question": "Qual é a letra G?", "answer": "G"},
    {"question": "Qual é a letra H?", "answer": "H"}
]
